...
def _build_data_sets(self, words, max_vocab):
    ...
    word_frequency.extend(collections.Cournter(words).most_common(max_vocab - 1))
    #単語=>IDの辞書
    w_to_id = dict()
...
class DataSet(object):
    ...
    def __init__(self, data_dir, max_vocab):
        ...
        #ファイルを読み込み
        row_dicuments = [self._read_document(file_paht) for file_path in file_pathes]
        #必要な部分だけ抽出
        documents = [self._preprocissing(document) for document in row_documents]
        #形態素解析
...
#ミニバッチ作成2
    ...
    #データセットが1週思想ならindexを最初に戻す
    if self.data_index + span > len(self.id_sequence):
        self.data_index = 0
    #初期のqueueを構築（window内の単語を全て格納）
    ...
    for i in range(batch_size // num_skips):
        ...
        #次に入れる単語がデータセットにない場合はbuifferには最初の値を入力
        if self.data_index == len(self.id_sequence):
            buffer = self.id_sequence[:span]
            self.data_index = span
        else:
            #bufferに次の単語を追加してindexを１進める
            buffer.appen(self.id_sequence[self.data_index])
            self.data_index += 1
