...
tf.flags.DEFINE_string('img_embedding_pb', 'create_inception_v3/inception_v3_freezed_pb',
        "Image embedding network pb file path.")
tf.flags.DEFINE_string('model_dir', 'ckpt/', "Svaed checkpoint directory.")
...
tf.flags.DEFINE_integer('max_step', 100000, "Num of fmax train step.")
...
def main(argy):
    ...
    with infer_graph.as_default():
        #推論の入力はplaceholder形式
        input_img = tf.placeholder(tf.float32, [None, 299, 299, 3],
                name = 'input_img')
    ...
    with train_graph.as_default():
        ...
        #キャプション生成
        train_output, train_lengths = build_caption(train_embedding,
                vocab_size, decoder_input=decoder_input,
        ...
        #生成テキストのlogging
        text_path = tf.placeholder(tf.string, shape=(None,), name='generated')
...
def build_input(pattern, repeat_count=None, shuffle=True):
    ...
    fataset = tf.data.TFRecordDataset(files). map(_parse_function). repeat(count=repeat_count)
...
def _parse_function(sequence_example_proto):
    ...
    context, feature_lists = tf.parse_single_sequence_example(
        ...
        sequence_features = {
            "caption": tf.FixedLenFeature([], dtype=tf.int64),
        })
    #パース結果を格納、画像は加工して格納
...
def _process_img(encoded_img):
    ...
    padded_shapes = (tf.TensorShape([299, 299, 3]), tf.TensorShape([]),
...
def build_img_embedding(img_input, embedding_size):
    ...
    img_embedding = tf.import_graph_def(graph_def,
            input_map={'input_image': img_input},
            return_elements=['InceptionV3/Predictions/Reshape:0'])
...
#ヘルパー定義
def build_caption(img_embedding, vocab_size, is_train=True, decoder_input=None,
    decoder_lengths=None, end_token=EOS_ID):
    ...
    with tf.name_scope('captioning'):
        ...
        if is_train:
            ...
            helper = tf.contrib.seq2seq.TrainningHelper(embedded_input, decoder_lengths)
        else:
            #出力層で最も確率が高いものだけを選択し次の入力にするヘルパーを定義
            helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(
                word_embedding, tf.fill([batch_size], SOS_ID), end_token)
    ...
    def initialize_demol(saver, sess, initializer=None):
        ...
        last_step = sess_train.run(gloabl_step)
        ...
        if step % FLAGS.eval_interval == 0:
            ...
            summary_val = sess_train.run(summary_op, feed_dict={text_ph:captions},
                    optins=run_opt, run_metadata=run_meta)
